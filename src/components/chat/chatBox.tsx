            ...
            <Stack>
                {messages &&
                    messages.map((message, index) =>
                        <Stack key={index} className={`mr-1 ${message?.senderId === user?._id ? " message self align-self-end flex-grow-0" : "message allign-self-start flex-grow-0"
                            }`} style={{ color: 'white', marginLeft: '10px', marginBottom: '10px', maxWidth: '30%' }}>
